...
`|` 왼쪽이 **staged(초록)**, 오른쪽이 **작업 트리(빨강)** 입니다. untracked 파일은 작업 트리의 신규(`+`)로 잡힙니다. 브랜치 이름 색이 원격과의 관계를 나타냅니다 — 청록은 동일, 초록은 앞섬, 빨강은 뒤짐, 노랑은 갈라짐, 회색은 upstream 없음.

git 저장소가 아닌 곳에서는 대괄호 구역이 통째로 사라집니다.

### 사용량 구역

지표마다 자기 바 한 줄을 갖고, 앞에 아이콘과 이름이 붙어서 색이 같아도 줄이 구분됩니다.

```
🧠 Context  █████████░░░░░░░░░░░░░░░░░░░░░ 31% 311k/1M          이 대화의 컨텍스트 창
⏳ 5h quota ████░░░░░░░░░░░░░░░░░░░░░░░░░░ 14% (reset 9h24m)    계정의 5시간 사용량 한도
📅 7d quota ██████████████████░░░░░░░░░░░░ 61% (reset 2d23h)    계정의 7일 사용량 한도
```
...
한 줄로 압축하려면 `rateLayout: 'inline'` 으로 바꾸세요.
...
`█`과 `░`는 유니코드 블록에서 서로 다른 계열이라, **폰트에 따라 세로 위치가 어긋나 보입니다** — 안 채워진 부분이 채워진 부분보다 위로 떠 보이는 현상입니다. 그럴 때 `barStyle`을 바꾸면 됩니다.
...
`auto`는 트루컬러면 `solid`, 아니면 `block`을 씁니다.
...
| 옵션 | 기본값 | 설명 |
|---|---|---|
| `alwaysShowZeros` | `true` | posh-git처럼 `+0 ~0 -0`도 항상 표시. `false`면 0을 숨기고 깨끗할 때 `✔` |
...
| `barStyle` | `'auto'` | 바 글자. `'block'`은 █/░, `'line'`은 ━/─, `'solid'`는 전부 █에 밝기로만 구분. `'auto'`는 트루컬러면 solid, 아니면 block |
